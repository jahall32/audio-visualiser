...
function Analyzer({ sound, camera }) {
  ...
  //rotates meshes
  // --------- checklist --------
  // see if there can be shapes that come in overtime
  // allocate audio to certain frequesncies, i.e the hey hey bit

  useFrame(({ clock }) => {
    if (analyser.current) {
      const data = analyser.current.getAverageFrequency();

      mesh3.current.scale.x =
        mesh3.current.scale.y =
        mesh3.current.scale.z =
          (data / 130) * 2;
      mesh2.current.scale.x =
        mesh2.current.scale.y =
        mesh2.current.scale.z =
          (data / 100) * 2;
      ...
      //const mesh2Color = data > 82 ? "green" : "red";
      //mesh4.current.material.color.set(mesh2Color);

      mesh5.current.scale.x =
        mesh5.current.scale.y =
        mesh5.current.scale.z =
          (data / 150) * 2;
      mesh6.current.scale.x =
        mesh6.current.scale.y =
        mesh6.current.scale.z =
          (data / 60) * 2;
      //mesh7.current.scale.x = mesh8.current.scale.y = mesh.current.scale.z = (data / 250) * 2;

      //mesh rotations
      mesh.current.rotation.y = Math.sin(clock.elapsedTime * (data / 4000)) * 1;
      mesh2.current.rotation.x =
        Math.sin(clock.elapsedTime * (data / 500)) * 0.9;
